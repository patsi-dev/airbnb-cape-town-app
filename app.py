...
# --- WORKING HTTPS URL (OFFICIAL & STABLE) ---
@st.cache_data
def load_data():
    url = "https://data.insideairbnb.com/south-africa/wc/cape-town/2024-09-28/data/listings.csv.gz"
    try:
        df = pd.read_csv(url, compression='gzip', low_memory=False)
    except:
        st.error("Data loading failed. Using sample data...")
        # Fallback sample
        data = {
            'price': [1500, 2500, 1800, 3000, 1200],
            'room_type': ['Entire home/apt', 'Private room', 'Entire home/apt', 'Private room', 'Entire home/apt'],
            'accommodates': [4, 2, 3, 6, 2],
            'bedrooms': [2, 1, 1, 3, 1],
            'bathrooms_text': ['1 bath', '1 bath', '1 bath', '2 baths', '1 bath'],
            'neighbourhood_cleansed': ['City Bowl', 'Sea Point', 'Gardens', 'Camps Bay', 'Green Point']
        }
